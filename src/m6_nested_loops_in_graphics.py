...
def draw_L(window, circle, r, c):
    ...
    for k in range(r+3):
        for l in range(3):
            ...
            circle2.move_by(circle.radius*2*l,circle.radius*2*k)
            circle2.attach_to(window)
            window.render(.1)
    for y in range(c):
        for w in range(3):
            circle2=circle.clone()
            circle2.move_by(circle.radius*2*3+circle.radius*2*y,circle.radius*2*r+circle.radius*2*w)
            circle2.attach_to(window)
            window.render(.1)
...
def draw_wall_on_right(rectangle, n, window):
    ...
    for k in range(n):
        ...
        rectangle2.move_by(0, rectangle2.get_height() * k)
        for y in range(k+1):
            rectangle3=rectangle2.clone()
            rectangle3.move_by(rectangle.get_width()*-y,0)
